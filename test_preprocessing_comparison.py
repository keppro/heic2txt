#!/usr/bin/env python3
"""
Test script to compare OCR results with and without preprocessing
for IMG_7518.HEIC using the ground truth text from IMG_7518.txt
"""
...
def main():
    ...
    if not os.path.exists(heic_path):
        print(f"Error: HEIC file not found: {heic_path}")
        return
    ...
    # Save result
    output_file_no_preprocessing = os.path.join(output_dir, "IMG_7518_no_preprocessing.txt")
    ...
    # Save result
    output_file_with_preprocessing = os.path.join(output_dir, "IMG_7518_with_preprocessing.txt")
    ...
    if os.path.exists(png_path):
        os.remove(png_path)
        print(f"\n🧹 Cleaned up temporary file: {png_path}")
