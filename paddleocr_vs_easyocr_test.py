#!/usr/bin/env python3
"""
Comprehensive test script to compare PaddleOCR vs EasyOCR performance.
...
def test_ocr_engine(engine_name, image_path, ground_truth_path, language="en", preprocessing=False):
    ...
    Args:
        engine_name: Name of the OCR engine ('easyocr' or 'paddleocr')
...
def run_comprehensive_test():
    ...
    test_configs = [
        ...
        {'engine': 'paddleocr', 'preprocessing': False},
        {'engine': 'paddleocr', 'preprocessing': True},
    ]
    ...
    if successful_results:
        ...
        paddleocr_results = [r for r in successful_results if r['engine'] == 'paddleocr']
        ...
        if easyocr_results and paddleocr_results:
            ...
            paddleocr_no_prep = next((r for r in paddleocr_results if not r['preprocessing']), None)
            
            if easyocr_no_prep and paddleocr_no_prep:
                print(f"\n   Without Preprocessing:")
                print(f"   EasyOCR:   {easyocr_no_prep['similarity']:.2f}% ({easyocr_no_prep['total_time']:.2f}s)")
                print(f"   PaddleOCR: {paddleocr_no_prep['similarity']:.2f}% ({paddleocr_no_prep['total_time']:.2f}s)")
                
                if easyocr_no_prep['similarity'] > paddleocr_no_prep['similarity']:
                    diff = easyocr_no_prep['similarity'] - paddleocr_no_prep['similarity']
                    print(f"   🏆 EasyOCR is {diff:.2f}% more accurate")
                elif paddleocr_no_prep['similarity'] > easyocr_no_prep['similarity']:
                    diff = paddleocr_no_prep['similarity'] - easyocr_no_prep['similarity']
                    print(f"   🏆 PaddleOCR is {diff:.2f}% more accurate")
                else:
            ...
            paddleocr_with_prep = next((r for r in paddleocr_results if r['preprocessing']), None)
            
            if easyocr_with_prep and paddleocr_with_prep:
                print(f"\n   With Preprocessing:")
                print(f"   EasyOCR:   {easyocr_with_prep['similarity']:.2f}% ({easyocr_with_prep['total_time']:.2f}s)")
                print(f"   PaddleOCR: {paddleocr_with_prep['similarity']:.2f}% ({paddleocr_with_prep['total_time']:.2f}s)")
                
                if easyocr_with_prep['similarity'] > paddleocr_with_prep['similarity']:
                    diff = easyocr_with_prep['similarity'] - paddleocr_with_prep['similarity']
                    print(f"   🏆 EasyOCR is {diff:.2f}% more accurate")
                elif paddleocr_with_prep['similarity'] > easyocr_with_prep['similarity']:
                    diff = paddleocr_with_prep['similarity'] - easyocr_with_prep['similarity']
                    print(f"   🏆 PaddleOCR is {diff:.2f}% more accurate")
                else:
                    print(f"   🤝 Both engines have equal accuracy")
    
    # Save detailed results
    results_file = "paddleocr_vs_easyocr_results.json"
