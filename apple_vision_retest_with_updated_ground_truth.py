#!/usr/bin/env python3
"""
Apple Vision Retest with Updated EasyOCR Ground Truth
====================================================

This script retests Apple Vision OCR using the updated EasyOCR results
as the ground truth for more accurate comparison.
"""
...
def test_apple_vision_orientation(engine_name: str, image_path: str, orientation_desc: str, 
                                ground_truth: str, use_preprocessing: bool, language: str = "en") -> Dict:
    """Test Apple Vision with a specific orientation and preprocessing option."""
    ...
    try:
        ...
        # Calculate similarity with updated ground truth
...
def main():
    """Main test function."""
    print("🔬 Apple Vision Retest with Updated EasyOCR Ground Truth")
    print("=" * 70)
    ...
    print(f"Updated ground truth: {ground_truth_path}")
    print(f"Language: {language}")
    
    # Convert HEIC to PNG
    print("\n🔄 Converting HEIC to PNG...")
    png_path = heic_path.replace('.HEIC', '_retest.png')
    ...
    print(f"📄 Updated ground truth length: {len(ground_truth)} characters")
    print(f"📄 Ground truth preview: {ground_truth[:100]}...")
    ...
    # Test Apple Vision with each orientation and preprocessing option
    print(f"\n{'='*70}")
    print(f"Testing {engine.upper()} ENGINE with Updated Ground Truth")
    print(f"{'='*70}")
    
    for variant_path, orientation_desc, angle in variants:
        for use_preprocessing in preprocessing_options:
            result = test_apple_vision_orientation(
    ...
    # Print summary
    print("\n" + "=" * 120)
    print("📊 APPLE VISION RETEST RESULTS WITH UPDATED GROUND TRUTH")
    print("=" * 120)
    
    # Group results by preprocessing
    no_prep_results = [r for r in results if not r['preprocessing']]
    with_prep_results = [r for r in results if r['preprocessing']]
    
    print(f"\n🍎 APPLE VISION - NO PREPROCESSING:")
    print(f"{'Orientation':<20} {'Success':<8} {'Similarity':<10} {'Time (s)':<10} {'Text Length':<12}")
    print("-" * 70)
    for result in no_prep_results:
    ...
    print(f"{'Orientation':<20} {'Success':<8} {'Similarity':<10} {'Time (s)':<10} {'Text Length':<12}")
    print("-" * 70)
    for result in with_prep_results:
        status = "✅" if result['success'] else "❌"
        print(f"{result['orientation']:<20} {status:<8} {result['similarity']:<10.2f} "
              f"{result['total_time']:<10.3f} {result['text_length']:<12}")
    
    # Find best results
    best_no_prep = max(no_prep_results, key=lambda x: x['similarity']) if no_prep_results else None
    best_with_prep = max(with_prep_results, key=lambda x: x['similarity']) if with_prep_results else None
    
    print(f"\n🎯 BEST RESULTS:")
    if best_no_prep:
        print(f"   No Preprocessing: {best_no_prep['similarity']:.2f}% similarity with {best_no_prep['orientation']}")
    if best_with_prep:
        print(f"   With Preprocessing: {best_with_prep['similarity']:.2f}% similarity with {best_with_prep['orientation']}")
    
    # Overall best
    all_best = [r for r in [best_no_prep, best_with_prep] if r]
    if all_best:
        overall_best = max(all_best, key=lambda x: x['similarity'])
        print(f"\n🏆 OVERALL BEST:")
        print(f"   {overall_best['orientation']} - {'With' if overall_best['preprocessing'] else 'No'} Preprocessing")
        print(f"   Similarity: {overall_best['similarity']:.2f}%")
        print(f"   Time: {overall_best['total_time']:.3f}s")
        print(f"   Text length: {overall_best['text_length']} characters")
    ...
    for use_preprocessing in preprocessing_options:
        engine_results = [r for r in results if r['preprocessing'] == use_preprocessing]
        if engine_results:
            successful_orientations = [r for r in engine_results if r['success']]
            
            print(f"\n   {'With' if use_preprocessing else 'No'} Preprocessing:")
    ...
    if original_ground_truth:
        print(f"   Original ground truth: '{original_ground_truth}' ({len(original_ground_truth)} chars)")
        print(f"   Updated ground truth: {len(ground_truth)} chars")
        
        # Calculate similarity between ground truths
        gt_similarity = calculate_text_similarity(original_ground_truth, ground_truth)
        print(f"   Ground truth similarity: {gt_similarity:.2f}%")
    
    # Save detailed results
    results_file = "apple_vision_retest_results.json"
